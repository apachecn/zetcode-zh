...
Kotlin Swing 教程展示了如何在 Kotlin 中创建 Swing GUI 应用程序。 代码示例和图像可从作者的 Github [Kotlin-Swing](https://github.com/janbodnar/Kotlin-Swing) 存储库中获得。

Swing 是 Java 编程语言的主要 GUI 工具包。 它是 JFC（Java 基础类）的一部分，JFC 是用于为 Java 程序提供图形用户界面的 API。

## Kotlin 秋千 JFrame

在第一个示例中，我们在屏幕上显示一个基本窗口。 使用`JFrame`创建主应用程序窗口。
...
`KotlinSwingSimpleEx`继承自`JFrame`组件。 `JFrame`是顶级容器。 容器的目的是保存应用程序的组件。
...
`defaultCloseOperation`设置关闭框时默认发生的操作。 在我们的情况下，我们退出该应用程序。
...
使用`addActionListener()`将动作侦听器添加到按钮。 该操作通过调用`System.exit()`方法来终止应用程序。
...
该示例创建一个带有一个菜单项的简单菜单。 选择退出菜单项，我们关闭应用程序。
...
`JMenuItem`是一种特殊的按钮组件。 我们向它添加一个动作监听器； 选择后将终止应用程序。
