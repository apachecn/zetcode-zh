...
这是 Maven 构建文件。 我们具有以下依赖项：`logback-classic`用于记录日志，`spring-context`和`spring-core`是基本的 Spring 依赖项，`spring-test`用于测试，`hamcrest-all`包含 Hamcrest 匹配库的所有模块，而`JUnit`是用于单元测试的库。
