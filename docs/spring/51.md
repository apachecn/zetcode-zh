...
SpringRunner 教程展示了如何使用 SpringRunner 测试 Spring 应用程序。
...
在以下应用程序中，我们使用`SprigRunner`测试一个简单的服务。 该应用程序是一个 Spring 独立控制台应用程序。

该应用程序包含两个属性文件：一个文件用于生产应用程序，另一个文件用于测试。
...
测试类用`@RunWith(SpringRunner.class)`注释。 `@ContextConfiguration`定义了类级别的元数据，用于确定如何加载和配置用于集成测试的应用程序上下文。 此外，我们还提供了`@TestPropertySource`自定义测试属性文件。
...
在本教程中，我们展示了如何使用`SpringRunner`在 Spring 应用程序中创建测试。 [Spring MockMvc 教程](/spring/mockmvc/)， [Spring @PropertySource 教程](/spring/propertysource/)， [Java 教程](/lang/java/)。