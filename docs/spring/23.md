...
Spring @Configuration 注释教程展示了如何使用@Configuration 注释配置 Spring 应用程序。
...
以下应用程序使用`@Configuration`来配置 Spring 应用程序。
...
`AppConfig`是应用程序配置类。 它装饰有`@Configuration`注释，这是`@Component`的一种特殊形式。
...
应用程序类显示应用程序属性。 这些属性通过`@Value`注入到属性中。
...
在本教程中，我们使用`@Configuration`配置了一个 Spring 应用程序。
