...
以下应用程序使用`BeanPropertyRowMapper`从`coutries`表中读取所有行。 在示例中，我们使用 MySQL 数据库。
...
您可能也对这些相关教程感兴趣：经典的 Spring 应用程序中的 [JdbcTemplate](/articles/springjdbctemplate/) ， [Spring HikariCP 教程](/articles/springhikaricp/)和 [Java 教程](/lang/java/)，或列出了[整个 Spring 教程](/all/#spring)。