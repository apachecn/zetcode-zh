...
`WebApplicationInitializer`用于引导 Spring Web 应用程序。 `WebApplicationInitializer`注册一个 Spring `DispatcherServlet`并创建一个 Spring Web 应用程序上下文。 通常，开发人员使用`AbstractAnnotationConfigDispatcherServletInitializer`（它是`WebApplicationInitializer`的实现）来创建 Spring Web 应用程序。

传统上，基于 Servlet 的 Java Web 应用程序使用`web.xml`文件来配置 Java Web 应用程序。 从 Servlet 3.0 开始，可以通过 Servlet 上下文侦听器以编程方式创建 Web 应用程序。

## Spring WebApplicationInitializer 示例

以下应用程序使用`WebApplicationInitializer`创建 Spring Web 应用程序。
...
我们创建一个`AnnotationConfigWebApplicationContext`并向`register()`注册一个 Web 配置文件。 我们将应用程序上下文与 Servlet 上下文绑定。
...
在这里，我们注册一个 Spring `DispatcherServlet`，它是 Spring Web 应用程序的前端控制器。
...
在本教程中，我们使用`WebApplicationInitializer`创建了一个简单的 Spring Web 应用程序。
