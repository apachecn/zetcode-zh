...
该应用程序检索`dataSource` bean 并打印其属性。 激活属性是通过`System.setProperty()`方法设置的。 另外，我们可以将属性设置为 VM 变量`-Dspring.profiles.active=prod`或在 IDE 设置中。
...
我们在命令行上设置了配置文件的情况下运行应用程序

在本教程中，我们为 Spring 应用程序创建了开发和生产资料。
