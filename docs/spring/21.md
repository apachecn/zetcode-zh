...
该应用程序使用 Spring 的`@PropertySource`将`application.properties`文件中的属性包含到环境中，并将它们注入类属性中。
...
`AppConfig`是应用程序配置类。 `@PropertySource`将属性从`application.properties`注入到 Spring 的环境中。
...
在本教程中，我们展示了如何使用`@PropertySource`注释在 Spring 应用程序中方便地使用属性。
