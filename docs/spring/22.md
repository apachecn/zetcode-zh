...
Spring @ComponentScan 教程显示了如何在 Spring 应用程序中启用组件扫描。 通过组件扫描，可以通过 Spring 容器自动检测咖啡豆。
...
该应用程序允许使用`@ComponentScan`进行组件扫描。 我们有一个返回当前时间的服务 bean。
...
该应用程序带有`@ComponentScan`注释。 `basePackages`选项告诉 Spring 在`com/zetcode`包及其子包中查找组件。
