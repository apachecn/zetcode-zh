...
Spring Boot @Lazy 教程展示了如何使用 Spring @Lazy 注释懒惰地初始化 bean。

Spring 是用于创建企业应用程序的流行 Java 应用程序框架。 Spring Boot 是 Spring 框架的演进，可帮助您轻松创建独立的，生产级的基于 Spring 的应用程序。

## @懒

`@Lazy`注释指示是否要延迟初始化 bean。 它可以用于`@Component`和`@Bean`定义。 `@Lazy` bean 不会被初始化，直到被另一个 bean 引用或从`BeanFactory`中显式检索。 不使用`@Lazy`注释的 Bean 会被初始化。
...
`MyLazyBean`包含`@Lazy`批注。 第一次请求时，它会延迟地初始化。 从控制器请求。
...
在本教程中，我们展示了如何使用 Spring `@Lazy`批注。 您可能也对相关教程感兴趣：

*   [Spring Boot PostgreSQL 教程](/springboot/postgresql/)
*   [Spring Boot @Bean](/articles/springbootbean/)
*   [Spring Boot @Qualifier 批注](/articles/springbootqualifier/)