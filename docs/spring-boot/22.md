...
RESTful Web 服务的 Java API（JAX-RS）是 Java 编程语言 API 规范，它提供了根据代表性状态传输（REST）架构模式创建 Web 服务的支持。 JAX-RS 使用注释来简化 Web 服务客户端和端点的开发和部署。 JAX-RS 是 Java EE 的正式组成部分。
...
这是`HelloService`。 `@Path`批注定义了服务类将响应的 URL。 Spring 的`@Service`也注释了`HelloService`以进行自动检测。 我们的服务方法仅返回“ Hello from Spring”消息。
