...
Spring Boot @PostConstruct 教程显示了如何在 Spring 应用程序中使用`@PostConstruct`批注。

Spring 是流行的 Java 应用程序框架，而 Spring Boot 是 Spring 的演进，可以帮助轻松地创建独立的，生产级的基于 Spring 的应用程序。

## @PostConstruct

`@PostConstruct`是在依赖注入完成以执行任何初始化之后需要执行的方法上使用的注释。

## Spring Boot @PostConstruct 示例

以下应用程序演示了`@PostConstruct`的用法。 它使用注释创建两个日志方法，在初始化它们的 bean 之后调用它们。 这些消息在应用程序运行后显示。 应用程序本身向客户端发送一条消息。 从配置文件中读取文本消息。
...
`doLog()`方法用`@PostConstruct`注释修饰。 在初始化`MyController` bean 之后调用该方法。 它记录一条简单的参考消息。
...
从带有`@Value`批注的`application.properties`文件中读取返回给客户端的消息，并将其设置为`message`字段。
...
在本教程中，我们展示了如何在 Spring 应用程序中使用`@PostConstruct`批注。 您可能也对相关教程感兴趣： [Java Servlets 教程](/articles/javaservlet/)， [Spring Boot @Controller 教程](/springboot/controller/)， [Spring Boot @ExceptionHandler 教程](/springboot/exceptionhandler/)， [Spring Boot 上传 文件](/springboot/uploadfile/)， [Spring Boot @PathVariable 教程](/springboot/pathvariable/)， [Spring Boot @RequestParam 教程](/springboot/requestparam/)， [Spring Boot @ResponseBody 教程](/springboot/responsebody/)， [Java 教程](/lang/java/)。