...
在执行应用程序期间，许多事情可能出错。 磁盘可能已满，我们无法保存文件。 Internet 连接可能断开，我们的应用程序尝试连接到站点。 所有这些都可能导致我们的应用程序崩溃。 为了防止这种情况的发生，我们应该预期并应对预期程序操作中的错误。 为此，我们可以使用异常处理。
...
如果此人是未成年人，则会引发例外情况。 如果`raise`关键字没有特定的异常作为参数，则会引发`RuntimeError`异常，将其消息设置为给定的字符串。 该代码未到达`puts "Entry allowed"`行。 代码的执行被中断，并在救援块处继续执行。
