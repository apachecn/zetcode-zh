...
在`doDrawing()`方法中，我们计算面板的尺寸。 如果调整窗口大小，气泡将随机分布在窗口的整个区域。
...
当我们旋转恒星时使用`angle`。 `scale`因子确定恒星的大小。 最后，`delta`因子是刻度的变化量。
...
`GeneralPath`用于创建星形。 通过`moveTo()`方法将第一点添加到路径。 通过`lineTo()`方法添加 Star 的后续点。
