...
委托没有什么可以用常规方法完成的。 之所以使用代表，是因为它们带来了许多优点。 它们提高了应用程序和代码重用的灵活性。 像接口一样，委托使我们能够解耦和泛化我们的代码。 委托还允许将方法作为参数传递。 当我们需要确定在运行时调用哪种方法时，可以使用委托。 最后，委托人提供了一种无需对子类进行子类化就可以对它的行为进行专门化的方法。 类可能具有复杂的泛型行为，但仍应专门化。 类是通过继承或通过委托来专用的。
...
我们有四个班。 `FiveEventArgs`带有事件对象的一些数据。 `FEvent`类封装了事件对象。 `RandomEventGenerator`类负责生成随机数。 它是事件发送者。 最后，`ComplexEvent`是主要的应用程序类。
