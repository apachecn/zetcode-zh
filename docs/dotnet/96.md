...
范围是可以引用变量的范围。 在过程内部声明的变量具有过程范围。 仅在此特定过程中有效。 在模块内部声明的变量具有模块作用域。 它在模块中的任何地方都有效。
...
在前面的示例中，我们声明了两个变量。 变量 a 具有模块范围，变量 b 具有程序范围。
...
在前面的示例中，我们在两个不同的作用域中声明了一个具有相同名称的变量。 它们不会碰撞。 在`Main()`过程中声明的变量将覆盖在模块范围中声明的变量。
