...
本教程显示了如何使用`HttpWebRequest`，`WebClient`，`HttpClient`，`Flurl.Http`和`RestSharp`阅读页面。
...
在本文中，我们展示了如何使用 C# 阅读网页。 您可能也对以下相关教程感兴趣： [MySQL C# 教程](/db/mysqlcsharptutorial/)， [C# 中的日期和时间](/articles/csharpdatetime/)，[用 C# ](/articles/csharpreadtext/)或 [C# 读取文本文件 Winforms 教程](/gui/csharpwinforms/)。