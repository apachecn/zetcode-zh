...
本章专门介绍 Visual Basic 中的输入&输出。 Visual Basic 中的输入&输出基于流。

流是与输入&输出一起使用的对象。 流是字节序列的抽象，例如文件，输入/输出设备，进程间通信管道或 TCP / IP 套接字。 在 Visual Basic 中，我们有一个`Stream`类，它是所有流的抽象类。 还有一些其他类可以从 Stream 类派生而来，从而使编程更加容易。
...
在本章中，我们介绍了 Visual Basic 中的输入&输出操作。