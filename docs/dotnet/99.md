...
`NotOverridable`方法不能被覆盖，`NotInheritable`类不能从中继承。 这些关键字与应用程序设计有关。 我们不应从某些类继承，并且不应重写某些方法。
...
我们有四个班。 `FiveEventArgs`随事件对象一起传送一些数据。 `Five`类封装了事件对象。 `RandomGenerator`类负责生成随机数。 它是事件发送者。 最后是`Example`类，它是主要的应用程序对象，具有`Main()`方法。
