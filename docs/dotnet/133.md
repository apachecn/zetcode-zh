...
像人类语言一样，计算机语言也具有词汇结构。 C# 程序的源代码由令牌组成。 令牌是原子代码元素。 在 C# 中，我们具有注释，变量，文字，空格，运算符，定界符和关键字。
...
在上面的示例中，我们有一些文字。 `bool`文字可能具有值`true`或`false`。 “ James”是字符串文字。 `null`代表任何数据类型的默认值。 数字 23 是`Integer`文字。 数字 68.5 是浮点文字。 最后，1987 年 11 月 12 日是日期文字。
