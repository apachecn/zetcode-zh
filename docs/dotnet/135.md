...
使用`enum`关键字创建枚举。 星期一，星期二，...裸词实际上存储着数字 0..6。
...
不能为值类型分配`null`文字，可以给引用类型分配值。 使用数据库的应用程序处理空值。 因此，C# 语言中引入了特殊的可空类型。 可空类型是`System.Nullable&lt;T&gt;`结构的实例。
