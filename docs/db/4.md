...
我们已通过`deb`软件包下载了适用于 64 位体系结构的 Ubuntu Linux 的 MySQL 5.7.17 源。
...
`mysql_ssl_rsa_setup`工具会创建 SSL 证书和密钥文件以及 RSA 密钥对文件，以支持使用 SSL 进行安全连接以及使用 RSA 通过未加密连接进行安全密码交换（如果这些文件丢失）。
