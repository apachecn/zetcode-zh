...
时间戳 是一个字符序列，表示某个事件发生的日期和/或时间。 时间戳通常用于记录事件。 在 MySQL 中，我们有`TIMESTAMP`数据类型用于创建时间戳。 `TIMESTAMP`列可用于记录`INSERT`或`UPDATE`操作的日期和时间。 如果您自己没有给它一个值，它将自动设置为最近一次操作的日期和时间。 `TIMESTAMP`数据类型的范围为`1970-01-01 00:00:01` UTC 到`2038-01-19 03:14:07` UTC。
