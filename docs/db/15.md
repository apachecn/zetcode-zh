...
在上面的 SQL 语句中，我们将 Cars 表中的所有数据转储到 cars.csv 文件中。 `FIELDS TERMINATED BY`子句控制在文本文件中如何终止数据。 我们选择了一个逗号字符。 CSV 代表逗号分隔值，它是一种非常常见且可移植的文件格式。 它可以由许多其他应用程序（如 OpenOffice，其他数据库等）导入。
