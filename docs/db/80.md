...
PostgreSQL 通常在本地连接上使用信任或对等身份验证策略。 在信任身份验证策略的情况下，PostgreSQL 假定可以连接到服务器的任何人都可以使用他们指定的任何数据库用户名（甚至超级用户名）访问数据库。 与数据库建立连接时，不需要密码。 （在数据库和用户列中所做的限制仍然适用。）信任身份验证对于单用户工作站上的本地连接是适当的，并且非常方便。 它通常不适用于多用户计算机。 如果使用对等身份验证策略，则数据库用户名必须与操作系统用户名匹配。
