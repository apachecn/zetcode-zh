...
准备好的语句是可用于优化性能的服务器端对象。 准备查询后，将对其进行解析，重写和计划。 以后只需要执行准备好的语句。 因此，解析，重写和计划阶段仅执行一次，而不是每次执行语句时。 准备的语句仅在当前数据库会话期间持续。 会话结束时，准备好的语句将被遗忘，因此必须在重新使用之前重新创建它。
...
在这里，我们通过调用`pg_prepare()`函数准备查询。 函数的第二个参数是准备好的语句的名称。 每个连接必须唯一。 准备好的语句更快，并且可以防止 SQL 注入攻击。
...
`pg_execute()`函数发送一个请求，以执行带有给定参数的准备好的语句，并等待结果。 这些值绑定到占位符。
