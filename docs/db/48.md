...
`.tables`命令提供`test.db`数据库中的表的列表。 当前没有表格。 `.exit`命令终止`sqlite3`命令行工具的交互式会话。 `ls` Unix 命令显示当前工作目录的内容。 我们可以看到`test.db`文件。 所有数据将存储在该单个文件中。
...
`sqlite3_bind_int()`将整数值绑定到准备好的语句。 占位符将替换为整数 3。该函数的第二个参数是要设置的 SQL 参数的索引，第三个参数是要绑定到该参数的值。
