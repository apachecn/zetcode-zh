...
内联接是最常见的联接类型。 这也是默认的联接。 内联接仅从数据库表中选择具有匹配值的记录。 我们有 _ 内接头 _ 的三种类型：`INNER JOIN`，`NATURAL INNER JOIN`和`CROSS INNER JOIN`。 `INNER`关键字可以省略。
...
_ 外部联接 _ 不需要两个联接表中的每个记录都具有匹配的记录。 外部联接有三种类型。 左外部联接，右外部联接和完全外部联接。 在创建教程时，MySQL 不支持完全外部联接。

如前所述，内联接是最常见的联接。 外部联接对于查找孤立记录可能很有用。 如果一个人没有预订，是顾客吗？ 如果我们无法与客户匹配，预订是否有效？
...
这是两个表的正确联接的输出。 右侧表的所有记录（预订）在左侧均具有匹配的记录（客户）。
