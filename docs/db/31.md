...
我们检查返回值。 如果`mysql_init()`函数失败，我们将打印错误消息并终止应用。
...
我们使用`fseek()`函数将文件指针移到文件末尾。 我们将确定图像的大小。 如果发生错误，则设置错误指示器。 我们使用`fseek()`函数检查指示器。 如果发生错误，我们还将关闭打开的文件处理器。
...
我们使用`fwrite()`函数调用将检索到的数据写入磁盘。 我们使用`ferror()`函数检查错误指示符。
