...
Tcl 使用称为通道的对象读取和写入数据。 可以使用`open`或`socket`命令创建通道。 Tcl 脚本可使用三个标准通道，而无需显式创建它们。 操作系统会为每个新应用程序自动打开它们。 它们是`stdin`，`stdout`和`stderr`。 脚本使用标准输入`stdin`来读取数据。 脚本使用标准输出`stdout`写入数据。 脚本使用标准错误`stderr`来编写错误消息。
