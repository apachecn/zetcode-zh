...
除了`format()`功能外，我们还可以使用一些内置的转换功能。
...
我们有四个功能。 `toJSON()`是`toISOString()`的别名。
...
year 年是包含另一天的一年。 日历中额外一天的原因是天文日历年与日历年之间的差异。 我们需要添加`isLeapYear`插件。
