...
Ramda 教程展示了如何使用 Ramda 库，该库为 JavaScript 中的高级功能编程提供了工具。 在本教程中，我们可以互换使用术语列表和数组。

## 拉姆达

Ramda 是 JavaScript 程序员的实用功能库。 该库专注于不变性和无副作用的函数。 Ramda 函数也会自动进行更新，从而只需不提供最终参数就可以从旧函数中构建新函数。

在本教程中，我们在 Node 应用程序中使用 Ramda。
...
咖喱化是将需要多个参数的函数转换为另一个函数的过程，当提供较少的参数时，该函数将返回一个等待其余参数的新函数。
...
通过`pluck()`功能，我们获得`name`和`age`属性，并形成两个新列表。
...
通过在年龄列表上调用`Math.max()`功能，我们可以获得最老的年龄。
...
使用`splitEvery()`功能，我们可以将列表分成指定长度的块。
...
在这里，我们生成一个 2..11 值的列表。 我们使用`forEach()`功能浏览列表。
...
`sort()`函数返回列表的副本，该列表根据比较器功能排序。 比较器函数一次接受两个值，如果第一个值较小，则返回一个负数；如果较大，则返回一个正数；如果相等，则返回零。
...
通过`find()`和`propEq()`功能的组合，我们寻找具有指定属性的用户。
...
`filter()`功能根据提供的谓词功能过滤可过滤对象（例如列表或普通对象）。 （谓词是一个返回布尔值的函数）。
