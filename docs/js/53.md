...
Collect.js 是用于处理数组和对象的流畅，便捷的包装器。 它是 Laravel 藏品的港口。 它包含许多使数据处理更加容易的功能。
...
`dif()`功能将一个集合与另一个集合进行比较。 它从原始集合中返回第二个集合中不存在的值。
