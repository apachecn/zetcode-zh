...
Node.js 有一个名为`npm`的包管理器，它是一个庞大的开源 JavaScript 库生态系统。
...
`readdir()`异步读取当前工作目录的内容。 它用目录中文件的名称（不包括`.`和`..`）填充数组。
...
可以使用`npm`（节点包管理器）安装其他模块。 现在，我们将安装一个名为`builtin-modules`的新模块。 通过此模块，我们将列出所有可用的 Node.js 内置模块。
...
使用`npm install builtin-modules`，我们可以在本地安装`builtin-modules`模块。 创建一个新的`node_modules`目录，在其中存储模块及其依赖项。 npm 自动创建了一个`package-lock.json`文件。 它用于确保队友，部署和持续集成的依赖项安装的一致性。 该文件必须提交到源存储库。
