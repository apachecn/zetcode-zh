...
我们导入`route`装饰器和`run`函数。 `route`装饰器用于将函数绑定到请求 URL。 `run`函数启动服务器实例。 默认情况下，它是开发服务器。
...
通过`static_file()`函数，我们可以提供静态文件。 静态文件所在的目录在`root`参数中指定。
...
在应用中，我们从 MongoDB 集合中检索数据。 我们使用`template()`函数将模板文件与数据结合在一起。
