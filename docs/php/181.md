...
&lt;dfn&gt;PHP 数据对象（PDO）&lt;/dfn&gt;定义了用于访问 PHP 中的数据库的轻量级接口。 它提供了一个数据访问抽象层，用于在 PHP 中使用数据库。 它定义了用于各种数据库系统的一致 API。
...
&lt;dfn&gt;Doctrine&lt;/dfn&gt; 是一组 PHP 库，主要致力于在 PHP 中提供持久性服务。 它的主要项目是对象关系映射器（ORM）和数据库抽象层（DBAL）。
