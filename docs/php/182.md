...
在本文中，我们介绍了 PHP 中的文件系统功能。 我们处理文件和目录，确定文件许可权和可用磁盘空间，以及读写文件。 对于我们的示例，我们使用 PHP CLI。 [PHP 教程](/lang/php)是有关 ZetCode 的 PHP 语言的简要教程。

PHP 具有用于处理文件和目录的丰富功能集。 PHP 包含低级和高级文件系统功能。 `fopen()`功能是一个低级功能的示例； 它是类似 C 函数的薄包装。 `file()`函数是高级 PHP 文件系统函数的示例。
...
我们可能想使用一个不存在的文件，这可能发生。 `file_exists()`功能可用于防止这种情况。
...
我们使用`file_exists()`功能检查文件是否存在。 如果它不存在，我们将以一条消息终止脚本。
...
在下面的示例中，我们创建一个新文件，将其删除，然后检查其是否存在。 `touch()`功能设置文件的访问和修改时间。 如果该文件不存在，则会创建它。 `unlink()`功能删除文件。
...
在代码示例中，我们利用了所有三个功能：`file_exists()`，`touch()`和`unlink()`。
...
如果`touch()`功能失败，则会打印一条错误消息。 许多 PHP 函数在失败时会返回`false`值。
...
`copy()`功能复制文件，`rename()`重命名文件。 如果目标文件已经存在，它将被覆盖。
...
在此脚本中，我们使用`rename()`功能将`myfile2`文件重命名为`myfile_back`。

## E_WARNING

一些文件系统功能在失败时发出`E_WARNING`。 这是运行时警告（非致命错误）。 脚本的执行不会停止。

PHP 在这方面不一致。 并非所有文件系统功能都发出此警告-大多数功能仅在失败时才返回错误值。
...
使用`set_error_handler()`功能设置自定义错误处理程序。
...
`scandir()`是高级功能，用于列出指定路径内的文件和目录。 该函数从目录返回文件和目录的数组。
...
`fopen()`功能打开文件或 URL。 函数的第一个参数是文件名，第二个是打开窗口的模式。 例如，`'r'`模式打开仅用于读取，`'w'`模式仅用于写入。 如果我们以`'w'`模式打开文件，但该文件不存在，则会创建该文件。 可以在 [PHP 手册中的 fopen（）](http://php.net/manual/en/function.fopen.php)中找到模式列表。
...
该示例使用`fread()`功能读取整个文件，并将其输出到控制台。
...
在此示例中，我们使用`file()`功能一次读取了整个文件。 我们使用`foreach`循环遍历返回的数组。

`file_get_contents()`是另一个高级功能，它将整个文件读取为字符串。
...
该示例使用`file_get_contents()`功能一次读取了整个文件。 它以字符串形式返回数据。

## 读取格式化的数据

`fscanf()`功能根据格式分析文件的输入。 每次对`fscanf()`的调用都会从文件中读取一行。
...
我们阅读网页直至结束。 `feof()`用于测试网页的结尾。 使用`fread()`功能以 1 KB 的块读取页面。
...
`fopen()`功能以写入模式打开文件。 如果文件不存在，则会自动创建。
...
使用`fwrite()`功能，我们在文件中写入一行。 该函数将文件句柄作为其第一个参数。
...
我们在`myfile.txt`文件上运行三个功能。 该脚本检查当前用户的这些属性。
...
文件系统对不同的用户和用户组强制执行文件权限。 `fileperms()`功能获得文件许可； 它以数字方式返回文件的权限。
...
`fgetcsv()`功能从 CSV（逗号分隔值）文件中读取一行； 它返回一个包含读取字段的索引数组。 `fputcsv()`功能将一行格式设置为 CSV 并将其写入文件。
...
*   [PHP 文件系统功能](http://php.net/manual/en/ref.filesystem.php)

在本文中，我们介绍了 PHP 文件系统功能。