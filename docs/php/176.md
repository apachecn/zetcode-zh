...
教义 QueryBuilder 为创建和运行数据库查询提供了方便，流畅的界面。 它是对运行 SQL 语句的低级详细信息的抽象。 它可以使程序员避免过程的复杂性。

教义有两种查询构建器； 一个用于 ORM，一个用于 DBAL。 在本教程中，我们介绍了 DBAL 的 QueryBuilder。

教义数据库抽象层（DBAL）是位于 PDO 之上的抽象层，并提供了一种直观且灵活的 API，可以与最受欢迎的关系数据库进行通信。
