...
在此脚本中，我们在布尔上下文中检查一些值。 `var_dump()`功能显示有关变量的信息。 `(bool)`构建体称为转换。 在它的临时上下文中，0 值是一个数字。 在布尔上下文中，它为`False`。 布尔上下文是当我们使用（布尔）强制转换，何时使用某些运算符（否定，比较运算符）以及何时使用`if/else`，`while`关键字时。
