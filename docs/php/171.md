...
Monolog 记录器实例具有通道（名称）和处理器的堆栈。 处理器负责将消息保存到文件，数据库或将其发送到邮件。

记录的消息在处理器堆栈中传播。 最后一个处理器首先执行。 消息的进一步传播由`bubble`变量控制，该变量默认设置为`true`。
...
我们可以将多个处理器添加到堆栈中。 最后添加的处理器将首先执行。
...
> **注意**：Gmail 不是理想的测试应用程序。 我们应该使用诸如 Mailtrap 或 Mailgun 之类的在线服务，或者使用由网络托管公司提供的 SMTP 服务器。
