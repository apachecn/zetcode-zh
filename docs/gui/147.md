...
信号和插槽用于对象之间的通信。 当发生特定事件时，会发出信号。 插槽是正常的 C ++方法； 发出与其连接的信号时调用它。
...
在头文件中，我们声明了两个插槽。 `slots`不是 C ++关键字，它是 Qt4 扩展名。 在代码编译之前，这些扩展由预处理器处理。 当我们在类中使用信号和时隙时，必须在类定义的开头提供`Q_OBJECT`宏。 否则，预处理器会抱怨。
...
在`onCheck()`插槽内，我们通过单击按钮连接或断开`onClick()`插槽。
