...
在构造器中，我们启动`dx`变量并加载桨图像。 我们得到图像矩形并将图像移动到其初始位置。
...
在`Breakout`对象的构造器中，我们实例化了三十个砖块。
...
在`keyPressEvent()`方法中，我们监听与游戏相关的按键事件。 左和右光标键移动桨状对象。 他们设置`dx`变量，该变量随后添加到桨的 x 坐标中。 `P`键暂停游戏，空格键启动游戏。 `Esc`键退出应用。
