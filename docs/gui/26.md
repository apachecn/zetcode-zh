...
小部件是应用程序的基本构建块。 PyQt5 有各种各样的小部件，包括按钮，复选框，滑块或列表框。 在本教程的这一部分中，我们将描述几个有用的小部件：`QCheckBox`，处于切换模式的`QPushButton`，`QSlider`，`QProgressBar`和`QCalendarWidget`。

## QCheckBox

`QCheckBox`是具有两种状态的窗口小部件：开和关。 这是一个带有标签的盒子。 复选框通常用于表示可以启用或禁用的应用程序中的功能。
