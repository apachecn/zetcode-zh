...
当窗口小部件的窗口的大小，位置或栈发生更改时，将发出`configure-event`。
...
此代码确定复选框的状态。 根据状态，它通过`g_signal_connect()`函数连接回调或通过`g_signal_handler_disconnect()`函数断开连接。
...
我们使用`gtk_window_set_decorated()`函数删除窗口装饰。 这意味着该窗口将没有边框和标题栏。
...
在`on_button_press()`函数内，我们执行拖放操作。 我们检查是否按下了鼠标左键。 然后我们调用`gtk_window_begin_move_drag()`函数，该函数开始移动窗口。
...
此功能注册计时器。 `time_handler()`函数会定期重复调用。 就我们而言，每一秒钟。 将调用计时器函数，直到返回`FALSE`。
