...
我们使用`SetWindowPos()`功能将应用程序窗口放置在屏幕中央。
...
我们有一个带有两个子窗口的应用程序窗口。 两个子窗口具有蓝色和红色背景。
...
通常，通过按 `Escape` 键可以终止应用程序。 还显示一个消息框以确认终止。
...
询问用户是否确实要关闭应用程序是一种常见的做法。 如果我们有时钟或计算器，那就没关系了。 但是，如果我们有文本编辑器或绘图应用程序，那确实很重要。 我们可能不小心按了 `Escape` 键并失去了所有修改。
