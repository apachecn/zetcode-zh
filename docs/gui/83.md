...
`Canvas`是可以使用`GraphicsContext`提供的一组图形命令绘制的图像。 它是进行绘画的高级工具。
...
`setStroke()`方法设置当前的笔触绘画属性。 默认颜色是黑色。 `GraphicsContext`的笔触方法使用该属性。
...
在计算机图形学中，渐变是从浅到深或从一种颜色到另一种颜色的阴影的平滑混合。 在绘图和绘画程序中，渐变用于创建彩色背景和特殊效果以及模拟灯光和阴影。 有两种类型的渐变：线性渐变和径向渐变。
