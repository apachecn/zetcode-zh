...
事件是每个 GUI 应用不可或缺的一部分。 所有 GUI 应用都是事件驱动的。 应用会对在其生命周期内生成的不同事件类型做出反应。 事件主要由应用的用户生成。 但是它们也可以通过其他方式生成。 例如 Internet 连接，窗口管理器或计时器。 因此，当我们调用`MainLoop()`方法时，我们的应用将等待事件生成。 当我们退出应用时，`MainLoop()`方法结束。
