...
PyQt4 中的事件处理系统是通过信号&槽机制构建的。 如果单击按钮，将发出信号`clicked`。 该槽可以是 Qt 槽或任何可调用的 Python。 `QtCore.QCoreApplication`包含主事件循环。 它处理并调度所有事件。 `instance()`方法为我们提供了其当前实例。 注意，`QtCore.QCoreApplication`是用`QtGui.QApplication`创建的。 单击的信号连接到`quit()`方法，该方法终止应用程序。 通信是在两个对象之间进行的：发送者和接收者。 发送者是按钮，接收者是应用程序对象。
