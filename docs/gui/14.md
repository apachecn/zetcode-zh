...
在这里，我们介绍两个控件。 _ 组框 _ 是围绕一组控件的矩形。 这些通常是单选按钮。 组框具有描述控件的标签。 此控件的目的是对某种程度上相关的控件进行分组。 _ 单选按钮 _ 是一种特殊的按钮，用户可以选择但不能清除。 它允许用户从一组选项中选择一个独占选项。
...
在我们的示例中，我们有一个带有三个单选按钮的组框。 通过单击单选按钮，我们为右侧的矩形选择背景色。
...
_ 组合框 _ 是编辑框或静态文本与列表的组合。 当我们需要从可用选项列表中选择一个项目时，将使用一个组合框。
