...
`QWidget`小部件是 PySide 中所有用户界面对象的基类。 我们为`QWidget`提供了默认的构造器。 默认构造器没有父代。 没有父级的窗口小部件称为窗口。
...
面向对象编程中最重要的三件事是类，数据和方法。 在这里，我们创建一个名为`Example`的新类。 `Example`类继承自`QtGui.QWidget`类。 这意味着我们必须调用两个构造器。 第一个用于`Example`类，第二个用于继承类。 使用`super()`方法调用第二个。
...
以下是`QtGui.QPushButton`的构造器，我们将在示例中使用它。
...
我们创建一个按钮。 该按钮是`QtGui.QPushButton`类的实例。 构造器的第一个参数是按钮的标签。 第二个参数是父窗口小部件。 父窗口小部件是`Example`窗口小部件，通过继承它是`QtGui.QWidget`。
...
如果我们关闭`QtGui.QWidget`，则会生成`QCloseEvent`。 要修改小部件的行为，我们需要重新实现`closeEvent()`事件处理器。
