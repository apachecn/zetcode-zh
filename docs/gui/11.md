...
颜色对话框显示在`ShowColorDialog()`用户函数中。 该函数返回所选的颜色值。
...
获得颜色值后，我们调用`InvalidateRect()`函数。 此函数会将`WM_PAINT`消息发送到我们的子窗口。
...
在面板过程中，我们更改子窗口的背景色。 除了在窗口上显示文本之外，`ExtTextOut()`函数还可以更改窗口的背景色。 我们不会显示任何文本，我们只会更改背景颜色。 如果提供`ETO_OPAQUE`标志，则`ExtTextOut()`函数将使用`SetBkColor()`函数指定的颜色。
