...
在本章中，我们将使用 Java Swing 创建一个简单的益智游戏。 来源可从作者的 Github [存储库](https://github.com/janbodnar/Puzzle-game-in-Java-Swing)中获得。
...
从磁盘加载了 JPG 图像。 `ImageIO's` `read()`方法返回`BufferedImage`，这是 Swing 处理图像的重要类。
