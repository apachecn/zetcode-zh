...
在设计应用程序的 UI 时，我们决定要使用哪些小部件以及如何组织这些小部件。 为了组织窗口小部件，我们使用称为 _ 布局容器 _ 的专用非可见窗口小部件。 在本章中，我们将提及`GtkAlignment`，`GtkFixed`，`GtkVBox`和`GtkTable`。

## Gtk 修复

`GtkFixed`容器将子窗口小部件放置在固定位置并具有固定大小。 此容器不执行自动布局管理。 因此，它不适用于翻译，字体更改或主题。 在大多数应用程序中，我们不使用`GtkFixed`容器。 可能会有一些专门的区域可以使用容器（例如，定位图或图像）。
...
在我们的示例中，我们创建了三个按钮并将它们放置在固定坐标上。 当我们调整应用程序窗口的大小时，按钮将保持其大小和位置。
...
本示例将五个按钮打包到一栏中。 如果我们调整应用程序窗口的大小，则子窗口小部件也将被调整大小。
...
该对话框显示所有打开的窗口，或更确切地说是 JDeveloper 应用程序中的选项卡。
