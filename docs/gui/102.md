...
我们创建一个`wx.MenuItem`对象。 &字符指定加速键。 带“＆”号后面的字符带有下划线。 实际的快捷方式由字符组合定义。 我们指定了 `Ctrl` + `Q` 字符。 因此，如果我们按 `Ctrl` + `Q` ，我们将关闭应用程序。 我们在&字符和快捷方式之间放置一个制表符。 这样，我们设法在它们之间留出一些空间。 为了提供菜单项的图标，我们调用`SetBitmap()`方法。 通过调用`AppendItem()`方法将手动创建的菜单项附加到菜单。
