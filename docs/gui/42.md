...
在此代码行中，我们将`valueChanged()`信号连接到标签的内置`setNum()`插槽。 由于`setNum()`方法过载，因此我们使用`static_cast`选择正确的方法。
...
组合框的`activated()`信号插入标签的`setText()`插槽。 由于信号过载，我们进行静态转换。
...
我们在窗口上放置一个旋转框，并将其`valueChanged()`信号连接到`QLabel's` `setNum()`插槽。
