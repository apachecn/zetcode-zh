...
*   安全性增强的 CRT 功能
*   Windows API 内核和用户功能
*   Windows API Shell 轻量级实用程序功能
*   Windows API StrSafe 功能

建议您使用安全性增强的标准功能或 Windows API 安全功能。

## ANSI C 字符串函数

C 运行时（CRT）库函数的开销很小，因为它们在下面调用 Windows API 函数。 这些功能提供了可移植性，但有一些限制。 如果使用不当，可能会导致安全隐患。
...
安全性 CRT 功能为 CRT 功能增加了额外的安全性。 （它们不是标准功能，而是 MS 扩展。）这些功能验证参数，获取大小缓冲区，检查字符串是否以 NULL 终止以及提供增强的错误报告。
...
在示例中，我们展示了四个功能：`wcsnlen_s()`，`wcscpy_s()`，`wcscat_s()`和`wprintf_s()`。
...
`wcsnlen_s()`计算宽字符串的长度。 该功能仅检查前`MAX_CHARS`个字符。
...
使用`wcscpy_s()`功能，我们将`L"Wuthering"`字符串复制到缓冲区中。 该函数使用缓冲区中的最大字符数，如果失败，则返回错误代码。 该函数成功返回 0。
...
甚至还具有增强安全性的`wprintf()`功能； 它具有一些运行时约束。
...
这些功能特定于 Windows OS。 它们在`User32.lib`和`Kernel32.lib`中可用。 它们比 CRT 同类产品轻。
...
`lstrcmpW()`函数比较两个字符串。 如果字符串相等，则返回 0。 比较是区分大小写的。 这意味着“杯”和“杯”是两个不同的字符串。 `lstrcmpiW()`产生不区分大小写的字符串比较。 对于此功能，“杯子”和“杯子”相等。
...
在 C 编程中，用格式化的数据填充缓冲区至关重要。 `wsprintfW()`功能将格式化的数据写入指定的缓冲区。
...
我们有一句话。 `GetStringTypeW()`功能用于确定字符串的字符类型。
...
这些功能特定于 Windows OS。 它们在`Shlwapi.lib`中可用。
...
`StrStrW()`功能在句子中搜索单词。 如果成功，它将返回一个指向匹配子字符串的指针。
...
为了提高应用程序的安全性，发布了 StrSafe 功能。 这些功能需要目标缓冲区的大小作为输入。 保证缓冲区为空终止。 该函数返回错误代码。 这样可以进行正确的错误处理。

每个功能都有相应的字符计数`Cch`或字节计数`Cb`版本。

### 字符串长度

`StringCchLengthW()`和`StringCbLengthW()`功能可以确定字符和字节的字符串长度。
...
函数成功返回`S_OK`，失败返回`STRSAFE_E_INVALID_PARAMETER`。 如果`psz`中的值为`NULL`，`cchMax`大于`STRSAFE_MAX_CCH`或`psz`大于`cchMax`，则功能将失败。 `SUCCEEDED`和`FAILED`宏可用于检查函数的返回值。
...
使用`sizeof`运算符，我们获得字符数组的大小（以字节为单位）。 该值用作`StringCbLengthW()`功能的字符串中允许的最大字符数。
...
`StringCchPrintfW()`功能将格式化的数据写入目标缓冲区。
...
使用`StringCchPrintfW()`功能，我们将两个值插入目标缓冲区。
