...
工具提示是有关应用程序中项目的特定提示。 以下示例将演示如何在 Qt5 编程库中创建工具提示。
...
当我们点击按钮时，会产生一个`clicked`信号。 时隙是对信号做出反应的方法。 在我们的情况下，它是主应用程序对象的`quit`槽。 `qApp`是指向应用程序对象的全局指针。 它在`QApplication`头文件中定义。
