...
控件是 Windows 应用程序的基本构建块。 （控件在 UNIX 中称为小部件。）Windows API 教程的这一部分涵盖了静态控件，按钮，复选框和编辑框。
...
_ 静态控件 _ 显示文本和图形。 无法选择静态控件。 它还不能具有键盘焦点。
...
_ 按钮 _ 是带有文本标签的简单控件。 用于触发动作。 当我们单击一个按钮时，它会向其父窗口发送`WM_COMMAND`消息。 `wParam`参数的低位字包含控件标识符。
...
_ 编辑控件 _ 是一个矩形子窗口，用于输入和编辑文本。 它可以是单行或多行。
