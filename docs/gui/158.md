...
事件是任何 GUI 程序中的重要组成部分。 事件是由用户或系统生成的。 当我们调用应用程序的`exec_()`方法时，应用程序进入主循环。 主循环获取事件并将其发送到对象。 PySide 具有独特的信号和槽机制。
...
当我们调用应用程序的`exec_()`方法时，应用程序进入主循环。 主循环获取事件并将其发送到对象。 信号和槽用于对象之间的通信。 发生特定事件时会发出`signal`。 `slot`可以是任何 Python 可调用的。 当发出与其连接的信号时，将调用槽。
