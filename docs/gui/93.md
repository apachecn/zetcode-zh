...
在本章中，我们将展示 wxWidgets 中提供的几个小部件的小示例。 小部件是我们应用程序的构建块。 wxWidgets 包含大量有用的小部件。 _ 小部件 _ 是基本的 GUI 对象。 一个小部件为 wxWidgets 工具包命名。 该术语在 UNIX 系统上使用。 在 Windows 上，小部件通常称为控件。
