...
`wx.ScreenDC`用于在屏幕上的任何地方绘制。 如果要在整个窗口上绘制（仅 Windows），则使用`wx.WindowDC`。 这包括窗户装饰。 `wx.ClientDC`用于绘制窗口的客户区域。 客户区域是没有装饰（标题和边框）的窗口区域。 `wx.PaintDC`也用于绘制客户区。 但是`wx.PaintDC`和`wx.ClientDC`之间有一个区别。 仅可从`wx.PaintEvent`使用`wx.PaintDC`。 不应从`wx.PaintEvent`中使用`wx.ClientDC`。 `wx.MemoryDC`用于在位图上绘制图形。 `wx.PostScriptDC`用于在任何平台上写入 PostScript 文件。 `wx.PrinterDC`用于访问打印机（仅 Windows）。
...
在下面的几行中，我们介绍几个基本对象：颜色，画笔，笔，连接，顶盖和渐变。
...
<caption>Standard colour database</caption>
| 蓝晶 | 黑色 | 蓝色 | 紫罗兰色 | 棕色 |
| 凯迪蓝 | 珊瑚 | 暗蓝色 | 青色 | 深灰色 |
...
笔对象具有其他两个参数：_ 加入 _ 和 _ 上限 _。 连接定义线之间的连接如何绘制。 联接样式具有以下选项：
...
设备上下文可以分为几个部分，称为 _ 区域 _。 区域可以是任何形状，例如矩形或圆形。 使用`Union`，`Intersect`，`Substract`和`Xor`操作，我们可以创建复杂区域。 区域用于概述，填充和裁剪。
...
`Clipping`将绘图限制在特定区域。 裁剪通常用于创建效果并改善应用程序的性能。 我们使用`SetClippingRegionAsRegion()`方法将绘图限制在特定区域。
