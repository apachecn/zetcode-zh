...
在 Qt5 C ++编程教程的这一部分中，我们将讨论 Qt5 应用程序中的菜单和工具栏。
...
在我们的示例中，我们有一个包含三个动作的菜单。 如果我们选择退出操作，则实际上只有退出操作才可以执行某些操作。 我们还创建一个分隔符和`CTRL+Q`快捷方式，以终止应用程序。
...
在 C ++ Qt5 教程的这一部分的最后，我们创建了一个应用程序框架。 该示例主要基于`QMainWindow`小部件。
