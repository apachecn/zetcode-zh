...
在 Visual Basic Winforms 教程的这一部分中，我们将进行绘画。 当我们想要更改或增强现有控件时，将使用绘画。 或者，如果我们要从头开始创建自定义控件。 要进行绘画，我们使用 Winforms 库提供的绘画 API。 绘画是在一种方法中完成的，我们将其插入`Paint`事件。
...
在本章中，我们使用 Visual Basic 在 Winforms 中进行了一些绘画。