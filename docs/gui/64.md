...
所有 GUI 应用程序都是事件驱动的。 应用程序会对在其生命周期内生成的不同事件类型做出反应。 事件主要由应用程序的用户生成，但也可以通过其他方式生成，例如 Internet 连接，窗口管理器或计时器。
...
_ 事件源 _ 是状态更改的对象。 它生成事件。 _ 事件对象 _（事件）将状态更改封装在事件源中。 _ 事件侦听器 _ 是要通知的对象。 事件源对象将处理事件的任务委托给事件侦听器。

Java Swing 工具箱中的事件处理功能非常强大且灵活。 Java 使用事件委托模型。 我们指定发生特定事件时要通知的对象。

## 事件对象

当应用程序发生故障时，将创建一个事件对象。 例如，当我们单击按钮或从列表中选择一个项目时。 事件有几种类型，包括`ActionEvent`，`TextEvent`，`FocusEvent`和`ComponentEvent`。 它们每个都是在特定条件下创建的。
...
我们在面板上有一个关闭按钮。 它的侦听器在一个命名的内部类中定义。
...
下面的示例将从组件派生一个类，并在该类内部实现一个动作侦听器。
...
在此示例中，我们创建一个派生的 MyButton 类，该类实现了动作侦听器。
...
在此示例中，我们有一个按钮，一个微调器和一个状态栏。 对于一个事件，我们使用两个按钮侦听器。 一键单击将为微调器组件增加一年并更新状态栏。 状态栏将显示我们单击按钮的次数。
...
可以使用`removeActionListener()`方法删除注册的侦听器。 下面的示例演示了这一点。
...
面板上有三个组件：按钮，复选框和标签。 通过切换复选框，我们可以添加或删除按钮的侦听器。
...
Java Swing 教程的这一部分专门用于 Swing 事件。 我们介绍了事件源，事件对象，事件侦听器，创建事件处理程序的几种方法，多个源和侦听器，删除侦听器以及事件适配器。