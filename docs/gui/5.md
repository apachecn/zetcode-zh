...
在 Windows API 教程的这一部分中，我们介绍了系统功能。 系统功能接收有关系统的信息并以各种方式与系统通信。

## 屏幕尺寸

`GetSystemMetrics()`功能检索各种系统指标和系统配置设置。
...
我们使用`GetComputerNameW()`功能获得计算机的名称。 名称存储在`computerName`数组中。
...
`GlobalMemoryStatusEx()`功能在`MEMORYSTATUSEX`结构中存储有关内存状态的信息。
...
`GlobalMemoryStatusEx()`功能被执行； 信息存储在结构中。
...
最后，必须使用`CoTaskMemFree()`功能释放分配的内存。
...
驱动器名称是路径类型，因此`MAX_PATH`常量与其最大长度有关。 `LogicalDrives`是一个字符串数组，用作`GetLogicalDriveStringsW()`功能的缓冲区。
...
使用`wprintf()`功能将这三个金额打印到控制台。 这些值以 MB 表示。
...
在 Windows API 教程的这一部分中，我们使用了一些系统功能。