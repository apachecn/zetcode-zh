...
`Servlet`是 Java 类，可响应特定类型的网络请求-最常见的是 HTTP 请求。 Servlet 用于以 Java 实现 Web 应用程序。 它们在 Tomcat 或 Jetty 之类的 servlet 容器中运行。 如今，Java Web 程序员使用基于 Servlet 构建的框架来开发应用程序。
...
`maven-war-plugin`收集 Web 应用程序的所有工件依赖项，类和资源，并将它们打包到 Web 应用程序存档（WAR）中。 `jetty-maven-plugin`允许我们与`mvn jetty:run`一起运行嵌入式 Jetty 服务器。
