...
在 Java Servlet 文本教程中，我们展示了如何从 Java Servlet 返回纯文本。 该 Web 应用程序已部署在 Tomcat 服务器上。
...
以下 Web 应用程序使用 Java Servlet 将纯文本发送到客户端。 从资源目录中的文件中读取文本。
...
这是 Maven POM 文件。 我们有三个工件：用于 servlet 的`javax.servlet-api`，用于 Java JSON 处理的`gson`和`guava`通用库。 `maven-war-plugin`负责收集 Web 应用程序的所有工件依赖项，类和资源，并将它们打包到 Web 应用程序存档（WAR）中。
