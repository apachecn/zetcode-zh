...
在 Java servlet 发送图像教程中，我们使用 servlet 在 Java 中创建经典的 Web 应用程序。 Servlet 将图像发送到客户端。 Web 应用程序已部署在 Jetty 服务器上。
...
Eclipse Jetty 是 Java HTTP 服务器和 Java Servlet 容器。 Jetty 可以轻松地嵌入到设备，工具，框架，应用程序服务器和群集中。

## Java servlet 映像示例

以下 Web 应用程序将图像发送到客户端。 该 Web 应用程序使用 Java Servlet。
...
`javax.servlet-api`依赖性是用于构建 Java Servlet 的库。 `maven-war-plugin`收集 Web 应用程序的所有工件依赖项，类和资源，并将它们打包到 Web 应用程序存档（WAR）中。 `jetty-maven-plugin`插件对于使用 Jetty 服务器进行快速开发和测试非常有用。
...
`index.html`文件是我们应用程序的主页。 它具有一个链接，该链接调用一个 servlet 来管理图像文件。
