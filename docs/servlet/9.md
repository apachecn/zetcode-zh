...
这些是我们项目中使用的依赖项。 `javaee-web-api`是一组用于创建 Java Web 应用程序的 JAR。 `derbyclient`是 Derby 的数据库驱动程序。 `json-simple`库用于处理 JSON 数据。 本项目使用`guava`的辅助方法。 `derbyoptionaltools`包含一个帮助程序方法，该方法将数据库结果集转换为 JSON 格式。
...
`CarService`包含用于数据检索和修改的方法。 我们使用标准的 JDBC 代码。 Java 数据库连接（JDBC）是 Java 编程语言的应用程序编程接口（API），它定义了客户端如何访问数据库。
