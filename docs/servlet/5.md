...
Java Servlet 上传文件显示了如何使用 Servlet 技术在 Java Web 应用程序中上传单个文件。
...
在以下应用程序中，我们有一个 Web 表单来选择要上传到服务器的文件。 该表单调用 Java servlet，该 servlet 读取文件并将其保存到目录中。
...
这是一个 Maven Web 应用程序。 它部署在 Tomcat 上。
...
这是 Maven POM 文件。 `javax.servlet-api`工件用于 servlet。 `maven-war-plugin`负责收集 Web 应用程序的所有工件依赖项，类和资源，并将它们打包到 Web 应用程序存档（WAR）中。
