...
Enterprise JavaBean &lt;abbr&gt;（EJB）&lt;/abbr&gt; 是服务器端组件，封装了应用程序的业务逻辑。 EJB 在 EJB 容器中运行，该容器负责各种系统级服务，包括事务管理，安全性和并发控制。 EJB 是 Java EE 规范的一部分。
...
我们在 NetBeans 中创建一个新的 Web 应用程序。 该项目将称为`MyFirstEJB`。 从“服务器和设置”页面，选择 GlassFish 服务器，并将上下文更改为`myfirstejb`。

<figure>![Server and Settings](img/9e3fd63d24e08d352f163a16ee50a6f7.jpg)

<figcaption>Figure: Server and Settings</figcaption>

</figure>

在此对话框中，我们选择应用程序服务器，Java EE 版本和上下文路径。
...
这是我们的`index.html`页面。 如果我们访问应用程序的根页面，它将返回。

我们右键单击应用程序图标，然后选择一个 Session Bean 类型的新 EJB。 我们将 bean 称为`MyFirstBean`，键入`com.zetcode.ejb`包，然后选择无状态会话类型。

<figure>![Creating a new session bean in NetBeans](img/796134f0736a3fda2182cfb74bef9fba.jpg)

<figcaption>Figure: Creating a new session bean in NetBeans</figcaption>
...
<figure>![New servlet](img/19db17e82ce7b5bcf3426f53a74d998d.jpg)

<figcaption>Figure: New servlet creation</figcaption>
...
<figure>![MyFirstEJB project structure](img/ac0bbe5c4a0bbce3d4fd85b7cb344746.jpg)

<figcaption>Figure: MyFirstEJB project structure</figcaption>

</figure>

我们构建应用程序并将其部署到 GlassFish 服务器。 要构建应用程序，请右键单击项目图标，然后选择“ Build”（构建）。 要部署应用程序，请右键单击项目图标，然后选择“部署”。
...
使用`curl`工具，我们连接到`myfirstejb` Web 应用程序的`Greet` servlet，并向其传递参数。 Web 应用程序以问候语响应。
...
访问根页面，应用程序将返回 HTML 测试页面。

## 使用实体 bean 持久化数据

在第二个示例中，我们创建一个将读取和保存汽车的 Web 应用程序。 汽车对象将保存在 Derby 数据库中。

我们使用`car-app`名称创建一个新的 Java Web 应用程序。 然后，我们创建一个新的`Car`实体。 实体类文件类型位于持久性类别中。 软件包将为`com.zetcode.persistence`。 主键类型为`Long`，并选中了创建持久性单元选项。
...
<figure>![Persistence provider](img/765579fc12e1f045b195f1d303e60e23.jpg)

<figcaption>Figure: Persistence provider</figcaption>
...
每个实体都有一个唯一的对象标识符。 此唯一标识符或主键使客户端能够定位特定的实体实例。 `@Id`声明此实体 bean 的标识符属性，`@GeneratedValue`批注用于指定主键的生成方式，`@Column`将标识符映射到数据库表的`Id`列。
...
`persistence.xml`文件是一个配置文件，其中包含有关我们在应用程序中使用的数据库的信息。 `jdbc/sample`是 GlassFish 服务器随附的内置数据源。 在我们的应用程序中，我们将数据保存在 Derby 的预先创建的`sample`数据库中。 如果`eclipselink.ddl-generation`属性不存在，则会自动导致创建`Cars`表。

<figure>![NetBeans Derby tool](img/d0c532354f5a0e4d847c4915fdb72bc1.jpg)
...
<figure>![Apache Common Lang JARs](img/1c43118c3aff336f4ba6740eb07f49f5.jpg)

<figcaption>Figure: Apache Common Lang JARs</figcaption>
...
<figure>![Error pages](img/8c394d9b4f2908a0405b3b9bc81ad9ae.jpg)

<figcaption>Figure: Error pages</figcaption>
...
是时候部署应用程序了。 通过右键单击 Web 项目并选择 Deploy 命令来部署应用程序。 请注意，当我们清理项目时，将取消部署该应用程序。 如果未运行所选的应用程序服务器，那么 deploy 命令还将启动它。
...
GlassFish 的`asadmin`工具可用于确定当前已部署的应用程序。

GlassFish 启动时，NetBeans 会自动启动 Derby 服务器。 可以在 GlassFish 服务器设置中将其关闭。

<figure>![GlassFish server properties](img/7ffd32d22b24228122913724f8bb7f8d.jpg)

<figcaption>Figure: GlassFish server properties</figcaption>
...
我们部署应用程序。 我们看到警告语`Table/View 'CARS' already exists in Schema 'APP'`。 这是`eclipselink.ddl-generation`属性的结果，该属性设置为`create_tables`。 我们可以忽略警告。
