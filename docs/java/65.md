...
<figcaption>Figure: FreeMarker servlet example</figcaption>
...
在`my.properties`文件中，我们有一个键/值对。 该值是应用程序的版本。 该文件位于`usr/main/resources`目录中。
...
这是`index.ftl`文件。 它具有一个链接，该链接向服务器发送请求以获取应用程序的版本。
