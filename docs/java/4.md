...
像人类语言一样，计算机语言也具有词汇结构。 Java 程序的源代码由令牌组成。 令牌是原子代码元素。 在 Java 中，我们具有注释，标识符，文字，运算符，分隔符和关键字。
...
文字是类型的特定值的文字表示。 文字类型包括布尔值，整数，浮点数，字符串，空值或字符。 从技术上讲，文字将在编译时分配一个值，而变量将在运行时分配。
...
在上面的示例中，我们有几个文字值。 23 是整数文字。 “ James”是字符串文字。 `true`是布尔文字。 `null`是表示缺失值的文字。 68.5 是浮点文字。 “ J”是字符文字。
