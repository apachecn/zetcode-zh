...
操作员还原数字 7 的所有位。这些位之一还确定数字是否为负。 如果我们再一次对所有位取反，我们将再次得到 7。
...
| 操作员 | Meaning | 关联性 |
| --- | --- | --- |
| `[] () .` | 数组访问，方法调用，对象成员访问 | 左到右 |
| `++ -- + -` | 递增，递减，一元加减 | 右到左 |
| `! ~ (type) new` | 否定，按位非，类型转换，对象创建 | Right-to-left |
| `* / %` | 乘法，除法，模 | Left-to-right |
| `+ -` | 加，减 | Left-to-right |
| `+` | 字符串连接 | Left-to-right |
| `&lt;&lt; &gt;&gt; &gt;&gt;&gt;` | 转移 | Left-to-right |
| `&lt; &lt;= &gt; &gt;=` | 关系的 | Left-to-right |
| `instanceof` | 类型比较 | Left-to-right |
| `== !=` | 平等 | Left-to-right |
| `&` | 按位与 | Left-to-right |
| `^` | 按位异或 | Left-to-right |
| `&#124;` | 按位或 | Left-to-right |
