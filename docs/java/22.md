...
MVC 控制器是由`@Controller`装饰的 JAX-RS 资源方法。 MVC 控制器负责组合数据模型和视图（模板）以生成 Web 应用页面。 模型承载在视图中显示的数据。 使用`@Named`批注或通过注入`Models`接口创建模型。
...
`ApplicationConfig`是应用配置类。 从 Servlet 3.0 开始，可以省略`web.xml`文件。 在 Jersey 中，我们创建一个配置类来扩展抽象`Application`并使用`@ApplicationPath`批注。 `Application`定义 JAX-RS 应用的组件并提供其他元数据。 在这里，我们注册应用所需的资源类，提供程序或属性。
