...
休眠验证器是 Bean 验证的参考实现。 Hibernate Validator 允许表达和验证应用约束。 默认的元数据源是注释，可以通过使用 XML 进行覆盖和扩展。 它不依赖于特定的应用层或编程模型，并且可用于服务器和客户端应用编程。
...
<figure>![Project structure](img/389aad7d76fc1df2a5667f3f877f610f.jpg)
