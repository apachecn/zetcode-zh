...
`index.jsp`包含一个简单的 HTML 表单。 Stripes具有自己的标签。 `<stripes:errors/>`显示验证错误。 如果我们未在该字段中写入任何文本，则会显示验证错误。 在`<stripes:form>`标记中，我们指定应处理请求的操作 bean。 `<stripes:text/>`创建一个文本字段。 创建的请求参数将自动映射到`HelloActionBean`的`userName`属性。
