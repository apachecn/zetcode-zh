...
该示例创建一个 Java Swing 应用程序，并使用`ImageIcon`组件显示图像。
...
在命令行应用程序中，我们使用了`src/maimg/snake.jpg`路径。
...
该应用程序的当前工作目录是项目目录，在本例中为`DisplayImageEx`。 我们可以使用`user.dir`系统属性找出当前的工作目录。 `src`目录是项目目录的子目录。
...
这是 Java 教程中的 Displaying image。 我们已经构建了一个 Swing 应用程序，该应用程序使用命令行工具，Ant，NetBeans 和 Eclipse 显示图像。 您可能还需要查看 [ImageIcon 教程](/java/imageicon/)，[在 Java 中读写 ICO 文件](/articles/javaico/)， [Java Swing 教程](/tutorials/javaswingtutorial/)， [Java 2D 教程](/gfx/java2d/)， 或 [Java 游戏教程](/tutorials/javagamestutorial/)。