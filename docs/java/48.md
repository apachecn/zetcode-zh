...
Java forEach 教程显示了如何使用 Java 8 `forEach()`方法。 我们与消费者合作，并在列表，地图和集合集合上展示`forEach()`。
...
我们有一个字符串/整数对的映射。 使用`forEach()`方法，我们遍历地图并打印其键/值对。
...
在本教程中，我们介绍了 Java 8 `forEach()`方法。 我们已经介绍了使用者，并在列表，地图和集合上使用了`forEach()`。
