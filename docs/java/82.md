...
在示例中，我们使用迭代器迭代地图的键集。 在 while 循环中使用迭代器来遍历映射的键。 以后使用该键获取相应的值。
...
在此示例中，我们有一个国家及其首都的地图。 我们将地图过滤为仅包含值以 B 开头的对。
