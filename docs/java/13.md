...
这是`main()`方法。 它是每个控制台或 GUI Java 应用程序的入口。 该方法采用参数的字符串数组。
...
在方法内部声明的变量具有方法范围。 名称的范围是程序的区域，在该区域中可以引用名称声明的实体，而无需使用名称限定。 在方法内部声明的变量具有方法范围。 它也称为本地范围。 该变量仅在此特定方法中有效。
...
我们可能想要使用`@Override`批注，该批注指示编译器我们打算覆盖超类中的方法。 它有助于防止某些编程错误。
...
在`Derived`类中，我们可以覆盖`Base`类的`f1()`方法。 我们还使用`@Override`批注通知编译器我们正在重写方法。
