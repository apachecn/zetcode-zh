...
Log4j 具有一个特定的内置记录器，称为“根查询器”。 它位于层次结构的顶部，即使未配置，也始终存在。 它为应用程序中的所有类编写消息。 如果我们不希望将来自特定记录器的消息传递到根记录器，则将发信人的`additivity`属性更改为`false`。
...
*   OFF-最具体，不记录
*   致命-严重错误，将阻止应用程序继续； 非常具体，数据很少
*   错误-严重错误，可能可以恢复
*   警告-可能有害的消息
*   INFO-信息性消息
*   调试-常规调试事件
*   TRACE-细粒度的调试消息，通常捕获通过应用程序的流； 不太具体，很多数据
*   ALL-最不具体，所有数据
...
在第一个示例中，我们为一个简单的 Java 控制台应用程序设置了 Log4j。
...
模式布局由转换说明符组成。 每个说明符均以百分号开头，后跟可选的格式修饰符和强制转换字符。 `%d`输出记录事件的日期。 `%t`输出生成日志事件的线程的名称。 `%-5level`输出记录事件的级别，级别名称中至少要包含五个字符，并且这些字符必须对齐。 `%logger`输出发布了记录事件的记录器的名称。 `%m`打印与日志记录事件关联的应用程序消息，`%n`是平台相关的行分隔符或多个字符。
...
我们定义了两个记录器。 `com.zetcode`记录器登录到文件附加器。 根记录器未在此应用程序中使用。
...
下一个示例显示了如何在 Spring Boot 应用程序中使用 Log4j。 该应用程序是控制台 Java 程序。
...
在`Application`类中，我们设置了 Spring Boot 应用程序。
