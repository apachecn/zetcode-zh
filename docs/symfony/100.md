...
Symfony DBAL 教程显示了如何使用 Doctrine DBAL 组件在 Symfony 应用程序中使用数据库。
...
在下面的示例中，我们创建一个简单的 Symfony 应用程序，该应用程序使用 DBAL 读取数据。 我们使用 MySQL 数据库。
...
该 controller 方法执行一个查询，该查询从`countries`表中获取所有行，并将其作为 JSON 数据返回。 请注意，出于简单原因，我们已将查询生成器放入控制器中。 在生产应用程序中，还应该有一个服务层和一个存储库。
