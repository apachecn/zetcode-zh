...
我们安装`symfony/maker-bundle`和`symfony/web-server-bundle`。 这些对于开发模式很有用。 请注意，我们正在使用包的别名。
...
我们安装`twig-bundle`和注解。 注解位于`sensio/framework-extra-bundle`中。
...
跨站点请求伪造需要`symfony/security-csrf`包，`symfony/monolog-bundle`用于记录日志，`symfony/property-access`用于操纵 PHP 属性。
